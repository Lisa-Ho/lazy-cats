...
<g class="sankey-layer">
	<defs>
		{#each sankeyData.links as d, i}
			...
			>
				<stop offset="0%" stop-color={colorNodes(d.source) || "#ccc"}  />
				<stop offset="100%" stop-color={colorNodes(d.target) || "#ccc"} />
			</linearGradient>
		{/each}
	</defs>
	<g class="link-group">
	...
	<g class="rect-group">
		{#each sankeyData.nodes as d, i}
			...
			<text
				class="sankey-data-value"
				x={d.x0 < $width / 4 ? d.x1 - 18 : d.x0 + 9}
				y={(d.y1 + d.y0) /2}
				dy={fontSize / 2 -6}
				style="text-anchor: {d.x0 < $width / 4 ? 'start' : 'middle'};"
			>
				{d.value}
			</text>
		{/each}
